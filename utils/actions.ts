...
export const createAddProductActionUrl = async (
  ...
  try {
    ...
    return {
      message: `Error product not created a1: ${renderErrorUrl(error)}`,
    };
  }
  ...
  try {
    ...
    if (!product) {
      try {
        ...
        return {
          message: `Error product not created a2: ${renderErrorUrl(error)}`,
        };
      }
    }
    if (product) {
...
export async function addToCartActionProduct(
  prevState: any,
  formData: FormData
) {
  const user = await getAuthUser();
